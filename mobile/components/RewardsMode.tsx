...
import { Gift, Star, Check, X, Lock, Shield, Settings, Plus, DollarSign, Wallet, ShoppingBag, Trash2, Users, ChevronDown, User, Tag, AlertCircle, Package, Clock, Archive, ArrowLeft, Pencil, RefreshCw } from 'lucide-react';
...
export const RewardsMode: React.FC<Props> = ({ user, onUpdateLovedOne, onUpdateProfile, onRefreshMarket, onExit }) => {
  ...
  // User's own market items (for My Offers and My Requests tabs)
  const [myMarketItems, setMyMarketItems] = useState<MarketItem[]>([]);
  const [loadingMyItems, setLoadingMyItems] = useState(false);
  
  // Pending verification requests (for issuer)
  ...
  useEffect(() => {
    if (onRefreshMarket && selectedLovedOneId && selectedLovedOneId !== lastFetchedLovedOneId.current) {
      const lovedOne = user.lovedOnes.find(l => l.id === selectedLovedOneId);
  ...
  // Load user's own market items when entering vault mode
  useEffect(() => {
    if (viewMode === 'vault' && user.id) {
      loadMyMarketItems();
  ...
  const loadPendingVerifications = async () => {
    ...
    try {
      ...
      // Enrich with item information and find loved one
      const enriched = transactions.map((tx) => {
        // Find the loved one (holder)
        const holder = user.lovedOnes.find(lo => lo.id === tx.holder_id);
        
        // Get item details from metadata (snapshot at time of transaction)
        const itemTitle = tx.metadata?.title || 'Unknown Item';
        const itemIcon = tx.metadata?.icon || '🎁';
      ...
      console.warn('Failed to load pending verifications:', error);
  ...
  const handleApproveVerification = async (transactionId: string) => {
    try {
      ...
      // Reload verifications and market data
      await loadPendingVerifications();
      // Refresh loved ones' market data
  ...
  const handleRefreshTrades = async () => {
    ...
    await loadPendingVerifications();
    // Refresh market data for all loved ones to get updated transactions
    if (onRefreshMarket) {
      for (const lovedOne of user.lovedOnes) {
        await onRefreshMarket(lovedOne.id);
      }
    }
  };

  const loadMyMarketItems = async () => {
    if (!user.id) return;
    setLoadingMyItems(true);
    try {
      const marketResponse = await apiService.getUserMarket(user.id);
      const marketData = marketResponse.data as {
        items: Array<{
          ...
          cost: number;
          icon?: string;
          category: string; // 'SPEND' or 'EARN'
          is_active: boolean;
          visible_to_relationship_ids?: string[]; // Relationship IDs that can see this item
        }>;
        balance: number;
      ...
      const items: MarketItem[] = marketData.items
      ...
      console.warn('Failed to load my market items:', error);
      setMyMarketItems([]);
    } finally {
  ...
  const handleAddItem = async () => {
    ...
    if (viewMode === 'vault') {
      try {
        ...
        const allLovedOneIds = user.lovedOnes.filter(lo => !lo.isPending).map(lo => lo.id);
        ...
        if (!allSelected && selectedLovedOneIds.length > 0) {
          ...
          relationshipIds = selectedLovedOneIds.map(loId => {
            const lovedOne = user.lovedOnes.find(lo => lo.id === loId);
        ...
        <div className="bg-white border-b-4 border-slate-900 px-4 py-3 shrink-0 flex items-center justify-between relative z-10" style={{ paddingTop: 'calc(var(--sat, 0px) + 0.75rem)', marginTop: 'calc(-1 * var(--sat, 0px))' }}>
           <div className="flex items-center gap-3">
                <div>
                    ...
                    <p className="text-[9px] font-mono text-yellow-600 uppercase tracking-widest font-bold">Economy of Care</p>
                </div>
           </div>
                                    ...
                                    {myMarketItems.filter(item => item.category === 'spend').map(item => {
                                        ...
                                        const availableToNames = item.visibleToRelationshipIds && item.visibleToRelationshipIds.length > 0
                                            ? item.visibleToRelationshipIds.map(relId => {
                                                const lo = user.lovedOnes.find(l => l.relationshipId === relId);
                                                return lo?.name;
                            ...
                            <div className="space-y-3">
                                ...
                                {loadingVerifications ? (
                                    ...
                                    pendingVerifications.map((verification) => {
                                        ...
                                        const holder = user.lovedOnes.find(lo => lo.id === verification.holderId);
                        ...
                        {viewMode === 'vault' && (
                            <div>
                                ...
                                <div className="space-y-2 max-h-40 overflow-y-auto border-2 border-slate-200 p-2">
                                    ...
                                    {user.lovedOnes.filter(lo => !lo.isPending).map(lo => (
                                        <label 
                                            key={lo.id}
                                    ...
                                    {user.lovedOnes.filter(lo => !lo.isPending).length === 0 && (
